...
const ReservationDetail = ({ booking, formatPrice, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40" onClick={onClose}>
      <div
        className="w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-2xl border border-[#E8E0D1] dark:border-[#232737] bg-white dark:bg-[#161925] shadow-2xl"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex items-start justify-between gap-4 p-6 border-b border-[#E8E0D1] dark:border-[#232737]">
          <div className="min-w-0">
            <h3 className="text-lg font-semibold text-[#003844] dark:text-[#E9F1F2]">Reservation details</h3>
            <p className="text-xs text-[#879497] dark:text-[#6B828A] mt-0.5 break-all">ID: {booking._id}</p>
          </div>
          <div className="flex items-center gap-2 shrink-0">
            <Badge tone={STATUS_TONE[booking.status] || 'neutral'}>{booking.status}</Badge>
            <button onClick={onClose} className="p-1.5 rounded-lg hover:bg-[#F3ECDE] dark:hover:bg-[#232737] text-[#879497] dark:text-[#6B828A]">
              <X className="w-4 h-4" />
            </button>
          </div>
        </div>
        ...
        <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-x-8">
          <div>
            <p className="text-xs font-medium text-[#879497] dark:text-[#6B828A] uppercase tracking-wider mb-3">Guest</p>
            ...
            <p className="text-xs font-medium text-[#879497] dark:text-[#6B828A] uppercase tracking-wider mt-6 mb-3">Stay</p>
          ...
          <div>
            <p className="text-xs font-medium text-[#879497] dark:text-[#6B828A] uppercase tracking-wider mb-3">Payment</p>
            ...
            <p className="text-xs font-medium text-[#879497] dark:text-[#6B828A] uppercase tracking-wider mt-6 mb-3">Booking</p>
        ...
        {statusHistory.length > 0 && (
          <div className="px-6 pb-6">
            <p className="text-xs font-medium text-[#879497] dark:text-[#6B828A] uppercase tracking-wider mb-3">Status history</p>
            <div className="space-y-2">
              {statusHistory.map((h, i) => (
                <div key={i} className="flex items-start gap-3 text-sm">
                  <Clock className="w-3.5 h-3.5 mt-1 text-[#879497] dark:text-[#6B828A] shrink-0" />
                  <div className="min-w-0">
                    <p className="text-[#003844] dark:text-[#E9F1F2]">
                      <span className="text-[#879497] dark:text-[#6B828A]">{h.from || '—'}</span>
                      <span className="mx-1.5 text-[#879497] dark:text-[#6B828A]">→</span>
                      {h.to}
                    </p>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-[#E8E0D1] dark:border-[#232737] bg-[#FFFAF4] dark:bg-[#10131D]">
                    ...
                    <th className="py-3 px-4 text-left text-xs font-medium text-[#4D6166] dark:text-[#9FB2B8] uppercase tracking-wider">Hotel / Room</th>
                    <th className="py-3 px-4 text-left text-xs font-medium text-[#4D6166] dark:text-[#9FB2B8] uppercase tracking-wider">Dates</th>
                    <th className="py-3 px-4 text-left text-xs font-medium text-[#4D6166] dark:text-[#9FB2B8] uppercase tracking-wider">Total</th>
                    <th className="py-3 px-4 text-left text-xs font-medium text-[#4D6166] dark:text-[#9FB2B8] uppercase tracking-wider">Payment</th>
                ...
                <tbody>
                  {bookings.map((b) => (
                    <tr key={b._id} className="border-b border-[#E8E0D1] dark:border-[#232737] last:border-b-0 hover:bg-[#F3ECDE] dark:hover:bg-[#232737] transition-colors">
                      <td className="py-3 px-4">
                        <div className="flex items-center gap-3">
                          <div className="w-8 h-8 rounded-full bg-[#D4A853]/10 text-[#B58A2E] dark:text-[#E6C075] flex items-center justify-center text-xs font-semibold shrink-0">
                            {(b.guestDisplayName || b.user?.name || 'G').charAt(0).toUpperCase()}
                          </div>
                          <div className="min-w-0">
                            <p className="font-medium text-[#003844] dark:text-[#E9F1F2] truncate">{b.guestDisplayName || b.user?.name || 'Guest'}</p>
                            <p className="text-xs text-[#4D6166] dark:text-[#9FB2B8] truncate">{b.guestEmail || b.user?.email || '—'}</p>
                          </div>
                        </div>
                      </td>
                      <td className="py-3 px-4">
                        <p className="text-[#003844] dark:text-[#E9F1F2] truncate">{b.hotel?.name || '—'}</p>
                        <p className="text-xs text-[#4D6166] dark:text-[#9FB2B8] truncate">{b.room?.roomNumber ? `${b.room.roomNumber} · ${b.room.roomType}` : '—'}</p>
                      </td>
                      <td className="py-3 px-4 text-[#4D6166] dark:text-[#9FB2B8] text-xs whitespace-nowrap">
                        {dateFmt(b.checkInDate)} → {dateFmt(b.checkOutDate)}
                        {b.nights != null && <span className="text-[#879497] dark:text-[#6B828A]"> ({b.nights}n)</span>}
                      </td>
                      <td className="py-3 px-4 text-[#003844] dark:text-[#E9F1F2] font-medium whitespace-nowrap">{formatPrice(b.totalPrice)}</td>
                      <td className="py-3 px-4">
                        <p className="text-[#003844] dark:text-[#E9F1F2] text-xs">{b.paymentMethod || '—'}</p>
                        <p className={`text-xs ${b.isPaid ? 'text-green-600 dark:text-green-300' : 'text-[#879497] dark:text-[#6B828A]'}`}>
