...
const TestimonialsManagement = () => {
  ...
  const columns = useMemo(() => [
    columnHelper.accessor("name", {
      header: "Guest",
      cell: (info) => <span className="text-slate-900 dark:text-[#F2EFE8] font-medium">{info.getValue()}</span>,
    }),
    ...
    columnHelper.accessor("rating", {
      header: "Rating",
      cell: (info) => <span className="text-slate-900 dark:text-[#F2EFE8]">{info.getValue()}/5</span>,
    }),
    columnHelper.accessor("review", {
      header: "Review",
      enableSorting: false,
      cell: (info) => <span className="text-slate-600 dark:text-[#A9AEA7] max-w-md block">{info.getValue()}</span>,
    }),
        ...
        return (
          <div className="flex items-center gap-2">
            <button
              ...
              className="text-xs px-3 py-1.5 rounded-lg border border-black/[0.08] dark:border-[#303631] text-slate-500 dark:text-[#A9AEA7] hover:text-[#8A643F] dark:hover:text-[#C5A47E] hover:border-[#A67C52]/40 hover:bg-[#A67C52]/10 transition-all"
            >
              Edit
            </button>
            <button
              ...
              className="p-1.5 rounded-lg border border-black/[0.08] dark:border-[#303631] text-slate-400 dark:text-[#A9AEA7] hover:text-amber-600 dark:hover:text-amber-300 hover:border-[#A67C52]/45 dark:hover:border-[#303631]/45 hover:bg-[#EFEEE8] dark:hover:bg-[#222823] transition-all disabled:opacity-40"
              title={item.isVisible ? "Hide" : "Show"}
            >
              {togglingId === item._id ? (
                <div className="w-3.5 h-3.5 border-2 border-slate-300 dark:border-[#303631] border-t-transparent rounded-full animate-spin" />
              ) : item.isVisible ? (
  ...
  return (
    <div className="space-y-6 pb-10">
      <div>
        <h1 className="text-xl font-bold text-slate-900 dark:text-[#F2EFE8] tracking-tight">Testimonials</h1>
        <p className="text-sm text-slate-400 dark:text-[#A9AEA7] mt-1">Manage guest feedback visibility for your storefront.</p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="luxury-card p-5">
          <p className="text-xs uppercase tracking-[0.15em] text-slate-500 dark:text-[#A9AEA7]">Total Testimonials</p>
          <p className="mt-2 text-3xl font-bold text-slate-900 dark:text-[#F2EFE8]">{rows.length}</p>
        </div>
        <div className="luxury-card p-5">
          <p className="text-xs uppercase tracking-[0.15em] text-slate-500 dark:text-[#A9AEA7]">Visible</p>
          <p className="mt-2 text-3xl font-bold text-green-600 dark:text-green-300">{visibleCount}</p>
        </div>
        <div className="luxury-card p-5">
          <p className="text-xs uppercase tracking-[0.15em] text-slate-500 dark:text-[#A9AEA7]">Hidden</p>
      ...
      {editingId !== null && (
        <div className="luxury-card overflow-hidden">
          <div className="m-5 luxury-card-soft p-5">
            <h4 className="text-sm font-semibold text-slate-900 dark:text-[#F2EFE8] mb-3">Edit Testimonial</h4>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <div>
                <label className="text-xs text-slate-500 dark:text-[#A9AEA7]">Guest Name</label>
                <input
                  value={form.name}
              ...
              <div>
                <label className="text-xs text-slate-500 dark:text-[#A9AEA7]">Location</label>
                <input
                  value={form.address}
              ...
              <div>
                <label className="text-xs text-slate-500 dark:text-[#A9AEA7]">Rating (1-5)</label>
                <input
              ...
              <div className="md:col-span-2">
                <label className="text-xs text-slate-500 dark:text-[#A9AEA7]">Review</label>
                <textarea
                  rows="4"
                  value={form.review}
