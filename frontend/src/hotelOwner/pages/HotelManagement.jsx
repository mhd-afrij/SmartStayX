  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl font-bold text-slate-900 tracking-tight">Hotel Management</h1>
        ...
        <button
          ...
          className="gold-button px-4 py-2.5 text-xs"
        >
      ...
      {loading ? (
        <div className="rounded-2xl border border-black/[0.06] bg-white shadow-[0_20px_60px_rgba(15,23,42,0.06)] p-8">
          <div className="flex items-center gap-3">
            <div className="w-5 h-5 rounded-full border-2 border-[#2563EB]/30 border-t-[#2563EB] animate-spin" />
            <span className="text-sm text-slate-500">Loading hotel details...</span>
          </div>
        </div>
      ) : hotels.length === 0 ? (
        ...
        <form onSubmit={handleSubmit} className="grid gap-6 lg:grid-cols-[1fr_2fr]">
          ...
          <div className="space-y-4">
            <div className="rounded-2xl border border-black/[0.06] bg-white shadow-[0_20px_60px_rgba(15,23,42,0.06)] p-5 space-y-4">
              <div>
                <p className="text-sm font-medium text-slate-600 mb-1.5">Select Property</p>
              ...
              <div>
                <p className="text-sm font-medium text-slate-600 mb-1.5">Hotel Image</p>
                <label
                  htmlFor="hotel-image"
                  className="block border border-dashed border-black/[0.15] rounded-xl p-2 cursor-pointer hover:bg-[#f4f2ef] transition"
                >
                  {preview ? (
                    <img src={preview} alt="hotel preview" className="w-full h-48 object-cover rounded-lg" />
                  ) : (
                    <div className="w-full h-48 rounded-lg bg-[#f4f2ef] flex items-center justify-center text-slate-400 text-xs">No image</div>
                  )}
                </label>
          ...
          <div className="rounded-2xl border border-black/[0.06] bg-white shadow-[0_20px_60px_rgba(15,23,42,0.06)] p-5 space-y-4">
            ...
            <div>
              <p className="text-sm font-medium text-slate-600 mb-1.5">Address</p>
              <input
                type="text"
                value={form.address}
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              <div>
                <p className="text-sm font-medium text-slate-600 mb-1.5">Phone</p>
                <input
                  type="tel"
                  value={form.contact}
              ...
              <div>
                <p className="text-sm font-medium text-slate-600 mb-1.5">Destination</p>
                <div className="relative">
                  <select
                    value={form.city}
            ...
            <div>
              <p className="text-sm font-medium text-slate-600 mb-1.5">Hotel Details</p>
              <textarea
      ...
      {hotels.length > 0 && (
        <div className="rounded-2xl border border-black/[0.06] bg-white shadow-[0_20px_60px_rgba(15,23,42,0.06)] p-5 space-y-4">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-1">Custom feature cards displayed on your room detail pages.</p>
          </div>

          {features.length > 0 && (
            <div className="space-y-2">
              {features.map((feat, index) => (
                <div key={index} className="flex items-center gap-3 bg-[#f4f2ef] rounded-xl p-3 border border-black/[0.06]">
                  <GripVertical className="w-4 h-4 text-slate-300 shrink-0" />
                  {(() => {
                    const Icon = ICON_MAP[feat.icon]
                    return Icon ? (
                      <Icon className="w-8 h-8 text-[#2563EB] shrink-0" />
                    ) : (
                      <div className="w-8 h-8 rounded-lg bg-white shrink-0" />
                    )
                  })()}
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-slate-900 truncate">{feat.title}</p>
                    <p className="text-xs text-slate-500 truncate">{feat.description || "No description"}</p>
                  </div>
                  <button
                    type="button"
          ...
          <div className="grid gap-3 sm:grid-cols-[120px_1fr_1fr_auto] items-end">
            <div>
              <p className="text-xs font-medium text-slate-500 mb-1">Icon</p>
              <select
                value={newFeature.icon}
                onChange={(e) => setNewFeature((prev) => ({ ...prev, icon: e.target.value }))}
                className="luxury-select w-full appearance-none px-2 py-2 text-xs"
              >
                {AVAILABLE_ICONS.map((ico) => (
                  <option key={ico.key} value={ico.key}>{ico.label}</option>
                ))}
              </select>
            </div>
            <div>
              ...
              <input
                ...
                className="luxury-input px-2 py-2 text-xs"
      ...
      {hotels.length > 0 && (
        <div className="rounded-2xl border border-black/[0.06] bg-white shadow-[0_20px_60px_rgba(15,23,42,0.06)] p-5 space-y-4">
          <div>
            <h3 className="text-lg font-semibold text-slate-900">Amenity Options</h3>
            <p className="text-sm text-slate-500 mt-1">Amenity choices available when adding or editing rooms for this hotel.</p>
          </div>
          ...
          {amenityOptions.length > 0 && (
            <div className="flex flex-wrap gap-2">
              {amenityOptions.map((opt, index) => (
                <div
                  key={index}
                  className="flex items-center gap-2 px-3 py-1.5 rounded-lg border border-black/[0.06] bg-[#f4f2ef]"
                >
