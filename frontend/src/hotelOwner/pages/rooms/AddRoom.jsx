...
const AddRoom = () => {
  ...
  return (
    ...
    >
      <div>
        <h1 className="text-xl font-bold text-slate-900 dark:text-[#F2EFE8] tracking-tight">Add Room</h1>
      ...
      <form onSubmit={onSubmitHandler} className="rounded-2xl border border-black/[0.06] dark:border-[#303631] bg-white dark:bg-[#1A1E1B] shadow-[0_20px_60px_rgba(24,59,53,0.06)] p-6">
        <div className="grid gap-6 lg:grid-cols-3">
          <div className="lg:col-span-2 space-y-6">
            <div>
              <p className="text-sm font-medium text-slate-600 dark:text-[#A9AEA7] mb-2">Images</p>
              <p className="text-xs text-slate-400 dark:text-[#A9AEA7] mb-3">Upload up to 4 photos (first will be the cover).</p>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                {Object.keys(images).map((key) => (
                  <label
                    htmlFor={`roomImage${key}`}
                    key={key}
                    className="relative border border-dashed border-black/[0.15] dark:border-[#303631] rounded-xl aspect-video flex items-center justify-center bg-[#EFEEE8] dark:bg-[#111412] hover:border-[#A67C52]/50 transition cursor-pointer overflow-hidden group"
                  >
                    {key === "1" && (
                      <span className="absolute top-2 left-2 z-10 text-[9px] uppercase tracking-wide bg-[#A67C52] dark:bg-[#C5A47E] text-[#1A1E1B] px-2 py-0.5 rounded-full font-medium">
                        Cover
                      </span>
                    )}
                    {images[key] ? (
                      ...
                      <div className="flex flex-col items-center gap-1 text-slate-400 dark:text-[#A9AEA7] group-hover:text-slate-600 dark:group-hover:text-[#F2EFE8] transition-colors">
            ...
            <div className="grid gap-4 md:grid-cols-3">
              ...
              <div>
                <p className="text-sm font-medium text-slate-600 dark:text-[#A9AEA7] mb-1.5">City</p>
                <input
                  type="text"
                  value={hotels.find(h => h._id === selectedHotel)?.city || ''}
                  disabled
                  className="luxury-input bg-[#EFEEE8] dark:bg-[#111412] text-slate-400 dark:text-[#A9AEA7]"
            ...
            <div>
              ...
              {!selectedHotel ? (
                <p className="text-xs text-slate-400 dark:text-[#A9AEA7]">Select a hotel first to see its amenity options.</p>
              ) : (
                      ...
                      <button
                        ...
                        className={`px-3 py-2 rounded-lg border text-xs font-medium transition-all ${
                          selected
                            ...
                            : "border-black/[0.08] dark:border-[#303631] bg-white dark:bg-[#1A1E1B] text-slate-500 dark:text-[#A9AEA7] hover:text-slate-700 dark:hover:text-[#F2EFE8]"
                        }`}
                      >
                        {amenity}
                      </button>
            ...
            <div className="flex items-center gap-3 pt-2">
              ...
              <p className="text-xs text-slate-400 dark:text-[#A9AEA7]">Ensure pricing and amenities are accurate before publishing.</p>
            </div>
          </div>

          <div className="rounded-xl border border-black/[0.06] dark:border-[#303631] bg-[#EFEEE8] dark:bg-[#111412] p-4 space-y-3">
            <p className="text-sm font-medium text-slate-600 dark:text-[#A9AEA7]">Quick tips</p>
            <ul className="space-y-1.5 text-xs text-slate-500 dark:text-[#A9AEA7]">
              {[
                ...
                <li key={i} className="flex items-start gap-2">
                  <span className="text-[#8A643F] dark:text-[#C5A47E] mt-0.5">•</span>
                  {tip}
                </li>
              ))}
            </ul>
