...
const HotelReg = () => {
  ...
  return (
    <div onClick={()=>setShowHotelReg(false)} className="fixed top-0 bottom-0 left-0 right-0 z-100 flex items-center justify-center bg-black/20">
      ...
      <form onSubmit={onSubmitHandler} onClick={(e)=>e.stopPropagation()} className="flex rounded-2xl md:max-w-4xl max-md:mx-2 overflow-hidden border border-black/[0.06] dark:border-[#303631] bg-white dark:bg-[#1A1E1B] shadow-[0_20px_60px_rgba(24,59,53,0.15)]">
        <img
          src={assets.regImage}
          alt="reg-Image"
        ...
        <div className="relative flex flex-col items-center md:w-1/2 p-8 md:p-10">
          ...
          <p className="text-2xl font-bold text-slate-900 dark:text-[#F2EFE8] tracking-tight mt-6">Register your Hotel</p>

          <div className='w-full mt-6 space-y-4'>
            <div>
              <label htmlFor='name' className='text-sm font-medium text-slate-600 dark:text-[#A9AEA7]'>Hotel Name</label>
            ...
            <div>
              <label htmlFor='contact' className='text-sm font-medium text-slate-600 dark:text-[#A9AEA7]'>Phone</label>
            ...
            <div>
              <label htmlFor='address' className='text-sm font-medium text-slate-600 dark:text-[#A9AEA7]'>Address</label>
            ...
            <div>
              <label htmlFor="city" className='text-sm font-medium text-slate-600 dark:text-[#A9AEA7]'>Destination</label>
              <select onChange={(e)=>setCity(e.target.value)} value={city} id="city" className="luxury-select text-sm mt-1.5 cursor-pointer" required>
                <option value="" className="bg-white dark:bg-[#1A1E1B]">Select Destination</option>
                {cities.map((city)=>(
