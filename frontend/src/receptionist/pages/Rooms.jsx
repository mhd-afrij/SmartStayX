  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6 pb-10">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl font-bold text-slate-900 dark:text-[#F2EFE8] tracking-tight">Rooms</h1>
          <p className="text-sm text-slate-500 dark:text-[#A9AEA7] mt-1">Add, edit, and manage hotel rooms</p>
        </div>
        <div className="flex items-center gap-2 bg-[#EFEEE8] dark:bg-[#222823] rounded-xl p-1 border border-black/[0.06] dark:border-[#303631]">
          <button onClick={() => { setTab("list"); setEditingRoom(null); resetForm(); }}
            className={`px-4 py-1.5 text-sm rounded-lg transition-all ${tab === "list" ? "bg-[#EFEAE1] dark:bg-[#222823] text-[#183B35] dark:text-[#8FB8A8] border border-[#A67C52]/45 dark:border-[#303631]/45" : "text-slate-500 dark:text-[#A9AEA7] hover:text-slate-800 dark:hover:text-[#F2EFE8]"}`}>
            All Rooms
          </button>
          <button onClick={() => { setTab("add"); setEditingRoom(null); resetForm(); }}
            className={`px-4 py-1.5 text-sm rounded-lg transition-all flex items-center gap-1.5 ${tab === "add" ? "bg-[#EFEAE1] dark:bg-[#222823] text-[#183B35] dark:text-[#8FB8A8] border border-[#A67C52]/45 dark:border-[#303631]/45" : "text-slate-500 dark:text-[#A9AEA7] hover:text-slate-800 dark:hover:text-[#F2EFE8]"}`}>
            ...
            <div className="p-5 border-b border-black/[0.06] dark:border-[#303631]">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <div className="w-8 h-8 rounded-lg bg-green-50 border border-green-200 dark:border-green-500/25 flex items-center justify-center">
                    <DoorOpen className="w-4 h-4 text-green-700 dark:text-green-300" />
                  </div>
                  <div>
                    <h3 className="text-sm font-medium text-slate-900 dark:text-[#F2EFE8]">All Rooms</h3>
                ...
                <div className="grid gap-4 p-5 md:grid-cols-2 lg:grid-cols-3">
                  {paginated.map((room) => (
                    <motion.div
                      ...
                      className="group rounded-2xl border border-black/[0.06] dark:border-[#303631] bg-white dark:bg-[#1A1E1B] overflow-hidden hover:border-black/[0.12] dark:hover:border-[#303631] shadow-sm transition-all"
                    >
                      {room.images && room.images[0] ? (
                        <div className="aspect-video bg-[#EFEEE8] dark:bg-[#222823] overflow-hidden">
                        ...
                        <div className="aspect-video bg-[#EFEEE8] dark:bg-[#222823] flex items-center justify-center">
                      ...
                      <div className="p-4 space-y-3">
                        <div className="flex items-start justify-between">
                          <div>
                            <h3 className="font-semibold text-slate-900 dark:text-[#F2EFE8]">{room.roomType || "Room"}</h3>
                            {room.hotel?.name && (
                              <p className="text-xs text-slate-500 dark:text-[#A9AEA7]">{room.hotel.name}</p>
                            )}
                            {room.roomNumber && (
                              <span className="inline-block mt-1 text-[10px] font-medium text-[#183B35] dark:text-[#8FB8A8] bg-[#EFEEE8] dark:bg-[#222823] px-2 py-0.5 rounded-full border border-[#A67C52]/45 dark:border-[#303631]/45">
                                Room {room.roomNumber}
                              </span>
                            )}
                            <p className="text-lg font-bold text-[#183B35] dark:text-[#8FB8A8] font-space mt-1">
                        ...
                        {room.amenities && room.amenities.length > 0 && (
                          <div className="flex flex-wrap gap-1">
                            {room.amenities.slice(0, 3).map((amenity, idx) => (
                              <span key={idx} className="text-[10px] bg-[#EFEEE8] dark:bg-[#222823] text-slate-500 dark:text-[#A9AEA7] px-2 py-0.5 rounded">
                                {amenity}
                              </span>
                            ))}
                        ...
                        <div className="pt-3 border-t border-black/[0.06] dark:border-[#303631] flex items-center justify-between">
                          <div className="flex items-center gap-1.5">
                            <button onClick={() => handleEdit(room)}
                              className="p-1.5 rounded-lg border border-black/[0.06] dark:border-[#303631] text-slate-400 dark:text-[#A9AEA7] hover:text-[#2A4A43] hover:border-[#183B35]/60 hover:bg-[#EFEAE1] dark:hover:bg-[#222823] transition-all">
                              <Edit3 className="w-3.5 h-3.5" />
                            </button>
                            <button onClick={() => handleToggle(room._id)}
                              className="p-1.5 rounded-lg border border-black/[0.06] dark:border-[#303631] text-slate-400 dark:text-[#A9AEA7] hover:text-amber-600 dark:hover:text-amber-300 hover:border-[#A67C52]/45 dark:hover:border-[#303631]/45 hover:bg-[#EFEEE8] dark:hover:bg-[#222823] transition-all">
                              {room.isAvailable ? <EyeOff className="w-3.5 h-3.5" /> : <Eye className="w-3.5 h-3.5" />}
                            </button>
                            <button onClick={() => handleDelete(room._id)}
                              className="p-1.5 rounded-lg border border-black/[0.06] dark:border-[#303631] text-slate-400 dark:text-[#A9AEA7] hover:text-red-600 dark:hover:text-red-300 hover:border-red-200 dark:hover:border-red-500/25 hover:bg-red-50 transition-all">
      ...
      {tab === "add" && (
        <div className="rounded-2xl border border-black/[0.06] dark:border-[#303631] bg-white dark:bg-[#1A1E1B] shadow-sm p-6">
          <div className="flex items-center gap-2 mb-6">
            <div className="w-8 h-8 rounded-lg bg-green-50 border border-green-200 dark:border-green-500/25 flex items-center justify-center">
              <Plus className="w-4 h-4 text-green-700 dark:text-green-300" />
            </div>
            <div>
              <h3 className="text-sm font-medium text-slate-900 dark:text-[#F2EFE8]">{editingRoom ? "Edit Room" : "Add New Room"}</h3>
              <p className="text-xs text-slate-500 dark:text-[#A9AEA7]">{editingRoom ? "Update room details" : "Fill in the details to add a new room"}</p>
            </div>
          </div>
          <form onSubmit={editingRoom ? handleUpdate : handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs text-slate-500 dark:text-[#A9AEA7] mb-1.5">Room Number *</label>
            ...
            <div>
              <label className="block text-xs text-slate-500 dark:text-[#A9AEA7] mb-1.5">Room Type *</label>
              <input type="text" value={form.roomType} onChange={(e) => setForm({ ...form, roomType: e.target.value })}
                className="luxury-input h-auto py-2 px-3 text-sm" />
            </div>
            <div>
              <label className="block text-xs text-slate-500 dark:text-[#A9AEA7] mb-1.5">Price Per Night *</label>
              <input type="number" value={form.pricePerNight} onChange={(e) => setForm({ ...form, pricePerNight: e.target.value })}
                className="luxury-input h-auto py-2 px-3 text-sm" />
            </div>
            <div>
              <label className="block text-xs text-slate-500 dark:text-[#A9AEA7] mb-1.5">Hotel *</label>
            ...
            <div className="md:col-span-2">
              <label className="block text-xs text-slate-500 dark:text-[#A9AEA7] mb-1.5">Amenities (comma separated)</label>
