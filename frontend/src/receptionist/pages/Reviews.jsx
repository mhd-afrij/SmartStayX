...
const satisfactionIcons = {
  ...
  satisfied: { icon: ThumbsUp, color: "text-[#5077B3] dark:text-[#93B3E0]" },
...
const Reviews = () => {
  ...
  const renderStars = (rating) => {
    return Array.from({ length: 5 }, (_, i) => (
      <Star key={i} className={`w-3 h-3 ${i < rating ? "text-amber-500 dark:text-amber-300 fill-amber-500" : "text-slate-300 dark:text-[#4E646B]"}`} />
    ));
  };
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6 pb-10">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-[#8299A0] mt-1">Moderate and manage guest reviews</p>
        </div>
      ...
      {loading ? (
        <div className="p-8 flex items-center gap-3">
          ...
          <span className="text-sm text-slate-500 dark:text-[#8299A0]">Loading reviews...</span>
        </div>
      ) : filtered.length === 0 ? (
        <div className="p-8 text-center text-slate-400 dark:text-[#6B828A] text-sm">No reviews found.</div>
      ) : (
          ...
          {paginated.map((review) => {
            ...
            return (
              <motion.div
                ...
                className="rounded-2xl border border-black/[0.06] dark:border-[#1D3842] bg-white dark:bg-[#122A32] shadow-sm p-5"
              >
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-center gap-3">
                    <div className="w-9 h-9 rounded-full bg-[#F4F2F9] dark:bg-[#1B2436] border border-black/[0.06] dark:border-[#1D3842] flex items-center justify-center">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-slate-400 dark:text-[#6B828A]">{review.hotel?.name} — Room {review.room?.roomNumber || ""}</p>
                    </div>
                  </div>
                ...
                {review.comment && (
                  <p className="text-xs text-slate-600 dark:text-[#9FB2B8] leading-relaxed mb-3">{review.comment}</p>
                )}

                <div className="flex items-center justify-between text-[10px] text-slate-400 dark:text-[#6B828A] pt-2 border-t border-black/[0.06] dark:border-[#1D3842]">
