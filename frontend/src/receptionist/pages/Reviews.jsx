...
const satisfactionIcons = {
  very_satisfied: { icon: Star, color: "text-green-600 dark:text-green-300" },
  satisfied: { icon: ThumbsUp, color: "text-[#183B35] dark:text-[#8FB8A8]" },
  neutral: { icon: Meh, color: "text-amber-600 dark:text-amber-300" },
  dissatisfied: { icon: Frown, color: "text-orange-600" },
  very_dissatisfied: { icon: Angry, color: "text-red-600 dark:text-red-300" },
};
...
const Reviews = () => {
  ...
  const renderStars = (rating) => {
    return Array.from({ length: 5 }, (_, i) => (
      <Star key={i} className={`w-3 h-3 ${i < rating ? "text-amber-500 dark:text-amber-300 fill-amber-500" : "text-slate-300 dark:text-[#A9AEA7]"}`} />
    ));
  };
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6 pb-10">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-[#A9AEA7] mt-1">Moderate and manage guest reviews</p>
        </div>
      ...
      {loading ? (
        <div className="p-8 flex items-center gap-3">
          ...
          <span className="text-sm text-slate-500 dark:text-[#A9AEA7]">Loading reviews...</span>
        </div>
      ) : filtered.length === 0 ? (
        <div className="p-8 text-center text-slate-400 dark:text-[#A9AEA7] text-sm">No reviews found.</div>
      ) : (
          ...
          {paginated.map((review) => {
            ...
            return (
              <motion.div
                ...
                className="rounded-2xl border border-black/[0.06] dark:border-[#303631] bg-white dark:bg-[#1A1E1B] shadow-sm p-5"
              >
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-center gap-3">
                    <div className="w-9 h-9 rounded-full bg-[#EFEEE8] dark:bg-[#222823] border border-black/[0.06] dark:border-[#303631] flex items-center justify-center">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-slate-400 dark:text-[#A9AEA7]">{review.hotel?.name} — Room {review.room?.roomNumber || ""}</p>
                    </div>
                  </div>
                  <button onClick={() => handleToggle(review._id)}
                    className={`p-1.5 rounded-lg border transition-all ${
                      review.isVisible
                        ? "border-green-200 dark:border-green-500/25 text-green-700 dark:text-green-300 hover:bg-green-50"
                        : "border-red-200 dark:border-red-500/25 text-red-600 dark:text-red-300 hover:bg-red-50"
                    }`}>
                ...
                {review.comment && (
                  <p className="text-xs text-slate-600 dark:text-[#A9AEA7] leading-relaxed mb-3">{review.comment}</p>
                )}

                <div className="flex items-center justify-between text-[10px] text-slate-400 dark:text-[#A9AEA7] pt-2 border-t border-black/[0.06] dark:border-[#303631]">
                  ...
                  <span className={`px-1.5 py-0.5 rounded-full border ${
                    review.isVisible ? "border-green-200 dark:border-green-500/25 text-green-700 dark:text-green-300" : "border-red-200 dark:border-red-500/25 text-red-600 dark:text-red-300"
                  }`}>
