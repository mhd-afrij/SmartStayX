...
const RoomDetails = () => {
  ...
  // Room fetch and booking form state.
  ...
  // Load the selected room from the API.
  useEffect(() => {
  ...
  // Restore any saved booking draft for this room.
  useEffect(() => {
    if (!room?._id) return;
  ...
  const updateField = (field, value) => {
    // Clear availability state whenever a field changes.
  ...
  const handleAvailabilitySubmit = async (event) => {
    // Step 1: verify availability, then create the booking if already confirmed.
    ...
    <div className="pt-24 pb-16 bg-slate-50">
      ...
      <div className="max-w-[1200px] mx-auto px-4 md:px-8 lg:px-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          {/* Main room gallery and booking form */}
