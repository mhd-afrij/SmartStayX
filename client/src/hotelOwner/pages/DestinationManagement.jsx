  ...
  return (
    ...
    >
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl font-bold text-white tracking-tight">Destination Management</h1>
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="rounded-2xl border border-white/[0.06] bg-white/[0.04] backdrop-blur-xl p-5 space-y-4">
          <div className="flex items-center justify-between">
            ...
            <button
              type="button"
              onClick={resetForm}
              className="p-1.5 rounded-lg border border-white/[0.06] text-white/30 hover:text-white/60 transition-all"
            >
              <X className="w-4 h-4" />
            </button>
          </div>
          ...
          <div className="grid gap-4 lg:grid-cols-[1fr_2fr]">
            <div className="space-y-4">
              <div>
                <p className="text-sm font-medium text-white/60 mb-1.5">Destination Image</p>
                <label
                  htmlFor="dest-image"
            ...
            <div className="space-y-4">
              <div className="grid sm:grid-cols-2 gap-4">
                <div>
                  <p className="text-sm font-medium text-white/60 mb-1.5">Destination Name</p>
                  <input
                    type="text"
                    value={form.name}
                ...
                <div>
                  <p className="text-sm font-medium text-white/60 mb-1.5">Country</p>
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full pl-8 pr-3 py-2.5 text-sm rounded-xl border border-white/[0.06] bg-white/[0.04] text-white/70 placeholder:text-white/30 outline-none focus:border-[#D4A85F]/30 transition-colors"
                      placeholder="e.g. Maldives"
                    />
                  </div>
                </div>
                <div>
                  <p className="text-sm font-medium text-white/60 mb-1.5">Hotel Count</p>
                  <div className="relative">
                ...
                <div>
                  <p className="text-sm font-medium text-white/60 mb-1.5">Temperature</p>
                  <div className="relative">
                    <Thermometer className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-white/30" />
                    <input
                      type="number"
              ...
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  ...
                  className="w-4 h-4 rounded border-white/[0.06] bg-white/[0.04] text-[#D4A85F] focus:ring-[#D4A85F]/30"
                />
                <Star className="w-3.5 h-3.5 text-[#D4A85F]" />
                <span className="text-sm text-white/60">Featured destination</span>
              </label>
              ...
              <div className="flex items-center gap-2 pt-2">
                ...
                <button
                  type="button"
                  onClick={resetForm}
                  className="px-4 py-2.5 text-sm font-medium rounded-xl border border-white/[0.06] text-white/50 hover:text-white hover:bg-white/[0.06] transition-all"
                >
                  Cancel
                </button>
              </div>
      ...
      <div className="rounded-2xl border border-white/[0.06] bg-white/[0.04] backdrop-blur-xl overflow-hidden">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-white/[0.08]">
                <th className="py-3 px-4 text-left text-xs font-medium text-white/50 uppercase tracking-[0.15em]">Image</th>
                <th className="py-3 px-4 text-left text-xs font-medium text-white/50 uppercase tracking-[0.15em]">Name</th>
                <th className="py-3 px-4 text-left text-xs font-medium text-white/50 uppercase tracking-[0.15em]">Country</th>
                <th className="py-3 px-4 text-left text-xs font-medium text-white/50 uppercase tracking-[0.15em]">Details</th>
                <th className="py-3 px-4 text-left text-xs font-medium text-white/50 uppercase tracking-[0.15em]">Featured</th>
                <th className="py-3 px-4 text-left text-xs font-medium text-white/50 uppercase tracking-[0.15em]">Actions</th>
              </tr>
            </thead>
            <tbody>
              {filteredDestinations.map((dest) => (
                ...
                >
                  ...
                  <td className="py-3 px-4 text-white/80 font-medium">{dest.name}</td>
                  <td className="py-3 px-4 text-white/60">{dest.country}</td>
                  <td className="py-3 px-4 text-white/60">
                    <span className="text-xs">{dest.hotelCount}</span>
                    {dest.temperature && (
                      <span className="text-xs ml-2 text-white/40">| {dest.temperature}</span>
                    )}
                  </td>
                  <td className="py-3 px-4">
                    {dest.featured ? (
                      <span className="text-[#D4A85F] text-xs">Featured</span>
                    ) : (
                  ...
                  <td className="py-3 px-4">
                    <div className="flex items-center gap-2">
                      <button
                        ...
                        className="p-1.5 rounded-lg border border-white/[0.06] text-white/30 hover:text-[#D4A85F] hover:border-[#D4A85F]/20 hover:bg-[#D4A85F]/10 transition-all"
                        title="Edit"
                      >
                      ...
                      <button
                        ...
                        className="p-1.5 rounded-lg border border-white/[0.06] text-white/30 hover:text-red-400 hover:border-red-400/20 hover:bg-red-400/10 transition-all disabled:opacity-40 disabled:pointer-events-none"
                        title="Delete"
                      >
        ...
        {loading && (
          <div className="flex items-center justify-center p-8">
            ...
            <span className="ml-3 text-sm text-white/40">Loading destinations...</span>
          </div>
        )}
        ...
        {!loading && filteredDestinations.length === 0 && (
          <div className="p-12 text-center">
            ...
            <p className="text-white/50 text-sm">No destinations yet. Create your first one!</p>
          </div>
        )}
      </div>
        )})()}
