...
export const calculatePrice = async (req, res, next) => {
  try {
    ...
    // Detect logged-in user (route is public, but auth middleware may set req.user)
...
export const createBooking = async (req, res, next) => {
  try {
    ...
    const { room, checkInDate, checkOutDate, guests, offerId, guestEmail } = req.body;
    const guestDisplayName = req.user?.name || 'Guest';
    const result = await bookingService.createBooking({ userId, room, checkInDate, checkOutDate, guests, offerId, guestDisplayName, guestEmail: guestEmail || req.user?.email });
  ...
  try {
    ...
    try {
      await transitionBookingStatus({
        ...
        options: { actor: user.toString(), reason: "User cancelled booking" },
      });
    } catch (transitionError) {
      return badRequest(res, transitionError.message);
    }
...
export const extendBookingHold = async (req, res, next) => {
  try {
    const user = req.user._id;
    const { bookingId, extendMinutes } = req.body;
    ...
    const result = await bookingService.extendHold({
      bookingId,
      userId: user,
      extendMinutes: extendMinutes ? Number(extendMinutes) : undefined,
    });

    return ok(res, { message: "Booking hold extended", ...result });
  } catch (error) {
    next(error);
  }
};
...
export const createCheckoutSession = async (req, res, next) => {
  try {
    ...
    booking.paymentMethod = booking.paymentMethod || "Stripe";
    await booking.save();
  ...
  try {
    ...
    if (event.type === "checkout.session.completed" || event.type === "checkout.session.async_payment_succeeded") {
      ...
      if (bookingId && session?.payment_status === "paid") {
        try {
          const paidBooking = await bookingService.confirmPayment({
            bookingId,
...
export const updateOwnerBookingPayment = async (req, res, next) => {
  try {
    ...
    if (!booking.paymentMethod) booking.paymentMethod = "Stripe";
...
export const updateOwnerBookingStatus = async (req, res, next) => {
  try {
    ...
    if (!canTransition(booking.status, status)) {
      return badRequest(res, `Invalid booking status transition: ${booking.status} → ${status}`);
    }

    await transitionBookingStatus({
      booking,
      to: status,
      options: { actor: ownerId.toString(), reason: "Owner updated booking status" },
    });
