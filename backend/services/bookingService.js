...
// Room-type (inventory) level availability — checks if there are free rooms
// of the same type at the hotel for the requested window.
const checkTypeAvailability = async ({ hotel, roomType, checkInDate, checkOutDate, excludeBookingId }) => {
  const window = normalizeBookingWindow(checkInDate, checkOutDate);
  if (!window) return { available: false, availableRooms: 0 };

  // Find all rooms of this type at the hotel
  const rooms = await Room.find({ hotel, roomType }).select('_id inventoryCount').lean();
  if (!rooms.length) return { available: false, availableRooms: 0 };

  // Count how many of these rooms already have overlapping active bookings
  ...
  // Total inventory = sum of inventoryCount across room docs of this type
  const totalInventory = rooms.reduce((sum, r) => sum + (r.inventoryCount || 1), 0);
  const availableRooms = Math.max(0, totalInventory - bookedCount);

  return { available: availableRooms > 0, availableRooms, totalInventory };
};
...
const createBooking = async ({ userId, room, checkInDate, checkOutDate, guests, holdMinutes, offerId, guestDisplayName, guestEmail }) => {
...
const confirmPayment = async ({ bookingId, stripeSessionId, stripePaymentIntentId }) => {
...
// Extend the hold on a pending booking to allow more time for payment
const extendHold = async ({ bookingId, userId, extendMinutes }) => {
  const booking = await Booking.findOne({ _id: bookingId, user: userId });
  if (!booking) throw Object.assign(new Error('Booking not found'), { status: 404 });

  if (booking.status !== BOOKING_STATUS.PENDING) {
    throw Object.assign(new Error('Only pending bookings can have their hold extended'), { status: 400 });
  }

  const now = new Date();
  if (booking.holdExpiresAt && new Date(booking.holdExpiresAt) <= now) {
    // Hold already expired — cannot extend after expiry
    throw Object.assign(new Error('Booking hold has expired and cannot be extended'), { status: 400 });
  }

  const maxExtensionMinutes = parseInt(process.env.BOOKING_MAX_HOLD_EXTENSION_MINUTES) || 30;
  const addMinutes = Math.min(
    extendMinutes || parseInt(process.env.BOOKING_HOLD_EXTEND_MINUTES) || 15,
    maxExtensionMinutes
  );

  const currentExpiry = booking.holdExpiresAt ? new Date(booking.holdExpiresAt).getTime() : now.getTime();
  const newExpiry = new Date(currentExpiry + addMinutes * 60 * 1000);

  booking.holdExpiresAt = newExpiry;
  await booking.save();

  return { holdExpiresAt: newExpiry, extendedByMinutes: addMinutes };
};
